...
A fifth typed entity, **EvidencePattern**, is declared in v0 as a supporting index of 213 patterns expressing the expected evidence shape for deterministic review. EvidencePattern is a first-class declarative class in the AppSec Core V1 ontology — `EvidencePatternShape` is defined in the apparatus SHACL skin (`appsec-core-v0-shapes.ttl`) at the cycle-close ontology release with `sh:targetClass ac:EvidencePattern`, plus optional property shapes referencing it from ControlObjective (`objective_verified_by_evidence_pattern`) and Artifact (`artifact_supports_evidence_pattern`). The class is intentionally unpopulated at V1: the SHACL conformance report records `target_node_count = 0` for `EvidencePatternShape`, and substrate v7 carries zero EvidencePattern claims (substrate v7 SUPPLIER SHA-256 `596783ed984d9c0e8c8ef6439a0eaee8fbaf2d863af37138cde8fad55d62be04`). The v0 supporting index of 213 evidence patterns — used by the programme's downstream retrieval runtime [6] — is preserved as the runtime-evidence-pattern artefact `evidence_patterns.json` within the cycle's deposited bundle (SHA-256 `89305ffcd279bdeb071bad48566cb96d11d524eeafdddd157ce6078bab95e394`; §15.0). The ontology declares the relationship via `appsec-core-evidence-pattern-index-v0.yaml`'s `source_alignment` block, which acts as the canonical pointer between the V1 declarative class and the v0 populated supporting surface. Population of the EvidencePattern class as first-class typed instances at the populated graph is registered as future programme work [4 §10.4]. The 234 / 259 figures reported throughout this paper for the v0 / v1 typed instance counts therefore refer to the four populated entity types only and exclude the EvidencePattern declarative class. Slices are governed by per-slice contracts specifying scope, non-goals, and acceptance conditions, and were derived bottom-up from a practitioner manual corpus through expert qualitative reading [1, §3]. Each slice has a declared primary control surface — the operational governance locus at which the slice's controls converge in the delivered software lifecycle — captured by a categorical descriptor (`single_broad_control_surface`, `dual_control_surface`, `multi_control_convergent_surface`, `split_control_surface`).
...
| ACR | Substantive name | Origin | Disposition | Slice landed/affected | Promoted entities |
|---|---|---|---|---|---|
| ACR-001 | Secure Configuration Baseline Integrity | Carried forward from v0 baseline (§5.2) | **Promoted** | Software Composition & Build Integrity | 3 ControlObjectives (ACO-RPR-008/009/010) + associated Practices and Mechanisms |
| ACR-002 | Security Requirements Lifecycle Management | Carried forward from v0 deferred-candidate set (§5.3) | **Promoted** | Threat Modelling (additive amendment for requirements-as-living-artefacts dimension) | 1 ControlObjective + 2 Practices + 2 Mechanisms |
| ACR-003 | Multi-anchor adjacency candidate batch | Surfaced by cycle re-application (§5.4) | **Not admitted** | — (identifier consumed) | — (no new entities; ~35 candidate items resolve as already-covered or Manual-only-routed) |
| ACR-004 | Output Rendering Safety / Context-Aware Encoding | Surfaced by cycle re-application (§5.5) | **Promoted** | Input/Output Validation & Filtering (additive amendment expanding scope from input-only to input/output) | 1 ControlObjective + 1 Practice + 1 Mechanism |
...
Three independent confirmation signals support the hypothesis:

- *Per-source coverage rate parity at established inverted-mapping baseline.* MITRE ATLAS reports 64.75% per-source coverage rate (GROUNDED items as fraction of source-extracted items, computed at substrate cycle close); CAPEC reports 63.51%. The two problem-space-inverted sources, mapped via the same method, produce parity coverage (+1.24 percentage points, within 2pp tolerance), indicating the method is method-stable across the two corpora.
- *Adjacency distribution parity.* The top-1 adjacency distribution under MITRE ATLAS reports a distribution shape congruent with CAPEC's top-1 distribution; the method's similarity-scoring is not noisier on the AI/ML problem-space-inverted source than on the established CWE/CAPEC counterparts.
- *Cross-corpus cluster convergence.* The largest detected convergence cluster (an AI-adversarial-reverse-engineering / model-stealing concept area attested across at least eight independent source families at substrate close) crosses ATLAS×CAPEC×additional families: the same conceptual cluster surfaces in both problem-space-inverted corpora under the same method. The method is method-stable across both AI/ML and pre-AI/ML problem-space-inverted sources.

H2 is reported as **confirmed**: no method refinement was required to integrate the AI/ML problem-space-inverted sources. The inverted-mapping method developed for CWE/CAPEC is method-stable for MITRE ATLAS and OWASP Top-10 for ML.
...
### 8.2 The three evidence pillars

Table 8 summarises the three evidence pillars; each pillar's evidence is reported in the section indicated.

**Table 8.** The three evidence pillars of *good-for-intended-fit*. Each pillar's evidence is computed at the cycle-close release tag (§15); per-source breakdowns are reported in the substrate's per-pilot evaluation records released with the cycle.
...
![Figure 1 — Multi-claim alignment between SSDF PW.8 and SP800-53 SA-11 (frontier-only match class).](images/figure-1-pw8-sa11-multi-claim-alignment.svg)
...
![Figure 2 — Multi-claim alignment between SSDF PO.1 and SP800-53 SA-1 (strict-match baseline).](images/figure-2-po1-sa1-strict-match-baseline.svg)
...
![Figure 3 — Multi-claim alignment between SSDF PO.2 and SCAGILE-OPS-15 (genuine divergence; honest disclosure).](images/figure-3-po2-ops15-genuine-divergence.svg)
...
- *Adversarial reasoning against worst-case constructions:* identification of a known adversarial test source-item (an item engineered to surface ontology blind-spots — e.g., novel substance under boundary-conflict slice contracts) whose pipeline-output is incoherent (top-1/top-2 differential below 0.05 with both anchors in incompatible slices) AND for which the curated decision cannot be justified under the ACR protocol's four conditions (§3.3).
...
The cycle's *good-for-intended-fit* acceptance criterion is satisfied at substrate cycle close under the three pillars:
...
**Schema conformance.** The cycle's per-pilot OLIR submissions pass **31 out of 31** against the official NIST OLIR JSON Schema 1.1 (schema id `https://csrc.nist.gov/1.1/olir_reference_document_json_1.1.schema`), validated under Python `jsonschema` 4.25.1 (Draft 4, matching the schema's declared `$schema`); the schema is archived locally for reproducibility. Zero relationships are dropped during translation (the empirical absence of below-E2-floor rows means no row would map to a non-enum value). The cycle's OLIR exports — one Reference Document for AppSec Core V1; thirty-one per-source crosswalks at the cycle's best-effort form (XML + JSON, sixty-six files); thirty-one schema-conformant per-pilot submissions plus one archived schema and validator reports (thirty-five additional files); and the deterministic translation script — are released within the cycle's deposited bundle (§15.0); the conversion methodology is documented as a bundle supplement.
...
[1] P. Farinha. *AppSec Core v0: A Normalized Ontology for Application Security*. Independent research preprint, 2025.

[2] P. Farinha. *Coverage-Preserving Compilation of a Practitioner Application-Security Manual: A Five-Source Case Study*. Independent research preprint, 2025.

[3] P. Farinha. *Security-by-Design Theory of Everything: Programme Statement*. Independent research preprint, 2025.

[4] P. Farinha. *AppSec Core v1: A Formalized Normalization Ontology for Application Security*. Independent research preprint (companion to this paper), 2026.
...
[6] P. Farinha. *Ontology-Grounded Retrieval for Auditable Large-Language-Model-Assisted Code Generation*. Independent research preprint, 2025.

[7] P. Farinha. *Empirical Evaluation: Deterministic Grounding versus Retrieval-Augmented Generation versus Ungrounded Compilation*. Pre-registered design (OSF DOI 10.17605/OSF.IO/H5AJE), 2026 [empirical execution in preparation].

[8] P. Farinha. *MCP Dual-Mode Instrument Specification*. Independent research preprint, 2026.
...
[35] Y. He, J. Chen, et al. "Ontology Matching with Large Language Models and Prioritized Depth-First Search". 2025. arXiv:2501.11441.

[36] J. D'Souza, N. Murphy, et al. "LLMs4OM: Matching Ontologies with Large Language Models". 2024. arXiv:2404.10317.
...
[39] H. B. Giglou, J. D'Souza, et al. "Large Language Models as Oracles for Ontology Alignment". 2025. arXiv:2508.08500.
