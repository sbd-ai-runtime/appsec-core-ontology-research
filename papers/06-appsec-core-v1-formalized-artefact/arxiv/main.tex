...
(multi-source convergence at ≥3 independent sources; multi-method
convergence; practitioner-Manual content backing; slice fit) and is
demonstrated through symmetric application across four worked decisions:
ACR-001 \emph{Secure Configuration Baseline Integrity} (promoted),
ACR-002 \emph{Security Requirements Lifecycle Management} (promoted),
ACR-003 \emph{Multi-anchor adjacency candidate batch} (not admitted),
and ACR-004 \emph{Output Rendering Safety / Context-Aware Encoding}
...
convergence} --- the candidate's substance appears across at least three
independent sources; (ii) \textbf{multi-method convergence} --- the
candidate is detected by independent normalization signals during the
cycle's re-application {[}4{]}; (iii) \textbf{practitioner-Manual
content backing} --- the candidate's substance has a counterpart in the
...
cross-source-convergent at the protocol's bar (≥3 independent sources)
and none was scope-amending at the slice level --- each refines within
existing contract scope and population. Their authority for inclusion is
the slice's existing per-slice contract; the cycle's per-source curation
{[}4{]} surfaced them as instance-level refinements consistent with
those contracts.
...
within a single organisational authority. The ACR protocol imposes no
organisational-authority gate --- authority spread is a descriptive
property of the corpus, not a promotion condition (§7.2 Condition 1) ---
so the threshold does not mitigate this dependency: partial vocabulary
co-evolution within and across authorities remains a property of the
corpus that multi-source convergence does not eliminate. The substantive
claim of C2 is therefore that the v0 schema absorbed the corpus's
\emph{partially-overlapping} multi-source pressure as additive instance
population, not that it absorbed thirty-one independent semantic
systems.
...
substance appears across \textbf{at least three independent sources}.
The burden is asymmetric in the strength of the claim the change
asserts: a promotion that introduces new Core entities (a
ControlObjective and its realizing chain, or Practices and Mechanisms,
optionally with an additive contract amendment) carries the burden of
\textbf{≥3 independent sources}; a change that would open a new
engineering slice carries the \textbf{extraordinary} burden of
\textbf{≥4 independent sources plus a demonstration of structural
inadequacy} in the existing partition --- the partition-level case
routed under Condition 4 to a separate governance exercise. The
three-source bar is calibrated against the cycle's first-wave bounding
test {[}2{]}: at three independent sources, convergence on the same
substance establishes that the candidate is not idiosyncratic to a
single team's interpretation. Convergence below three sources is a
detection-level signal that may motivate further investigation but does
not on its own meet the promotion bar. Organisational-authority spread
is a \textbf{descriptive property of the corpus, not a promotion
condition}: the threshold counts independent sources, and authority
diversity is reported where it bears on interpretation but never gates
admission. The threshold is \textbf{governance-calibrated rather than
mathematically optimal}: it operationalises the trade-off between
admitting clusters carried by single-pattern editorial framing (a lower
...
Sensitivity analysis under alternative thresholds (a two-source minimum;
...
convergence at ≥3 independent sources).} At promotion time the substance
converges across \textbf{4 independent sources} --- NIST SSDF PW.9,
OWASP ASVS V13, CIS Control 4, and MITRE CWE-1188/276/426--428 ---
meeting the protocol's ≥3-source bar for a new-Core-entity promotion.
Configuration baseline integrity is a near-universal AppSec topic and
the broader corpus carries it widely, but promotion rests on the four
named convergent sources rather than on corpus-wide spread. The full
...
\textbf{Per-condition evidence.} - \emph{Condition 1.} At promotion time
the substance converges across \textbf{5 independent sources} --- NIST
SSDF, OWASP SAMM (Security Requirements practice {[}16{]}), PCI SSLC,
NIST SP 800-53, and SAFECode Fundamental Practices for Secure Software
Development --- meeting the protocol's ≥3-source bar for a
new-Core-entity promotion. The broader corpus carries the substance more
widely, but promotion rests on the five named convergent sources. The
full per-source claim distribution is preserved in the substrate's
per-pilot configuration files at the cycle-close release tag (see {[}4
§15.2{]}). - \emph{Condition 2.} Multi-stage method convergence: the
structural-hierarchy stage places the candidate within the TMR slice;
the embedding-similarity stage returns coherent top-anchor cluster
across the convergent sources; the override-mechanism record concurs
with the method signal. - \emph{Condition 3.} Substance present in the
practitioner Manual under the threat-modeling chapter of the Manual's
prose layer. - \emph{Condition 4.} The candidate motivates an additive
amendment to the TMR slice's per-slice contract scope, declaring the
requirements-as-living-artefacts dimension as within scope alongside the
existing threat-side substance. The amendment is additive: prior scope,
non-goals, and acceptance conditions are preserved.
...
\textbf{Per-condition evidence.} - \emph{Condition 1.} At promotion time
the substance converges across \textbf{5 independent sources} --- OWASP
ASVS V5 {[}10{]}, MITRE CWE-79 {[}14{]}, MITRE CAPEC-63/242 {[}13{]},
OWASP MCP guidance, and OWASP DSOMM {[}17{]} --- meeting the protocol's
≥3-source bar for a new-Core-entity promotion. The promotion introduces
new Core entities and an additive contract amendment but does not open a
new slice, so the ≥3-source burden applies. The per-source claim
distribution is preserved in the substrate's per-pilot configuration
files at the cycle-close release tag (see {[}4 §15.2{]}). -
\emph{Condition 2.} Multi-stage method convergence: the
structural-hierarchy stage places the candidate within the IVF slice
with the slice-contract amendment proposed; the embedding-similarity
stage returns coherent top-anchor cluster across the convergent sources;
the override-mechanism record concurs with the method signal. -
\emph{Condition 3.} Substance present in the practitioner Manual under
the validation chapter of the Manual's prose layer. - \emph{Condition
4.} The candidate motivates an additive amendment to the IVF slice's
per-slice contract scope, expanding from ``input validation and
controlled failure'' to ``input/output validation and controlled
failure''. The amendment is additive: prior scope (input validation,
controlled failure under hostile or malformed input) is preserved; the
candidate adds the output-side substance.
...
carrying convergent substance across only two sources, one below the
three-source bar, or meeting Conditions 1, 2, and 3 but failing
Condition 4 by motivating a slice-contract amendment that could not be
defended without sibling-slice restructuring). ACR-003's non-admission
was a \emph{framing-level failure} (batch of approximately thirty-five
candidate items without a unifying substantive concept across the batch,
§9.1), categorically different from a single-condition marginal failure.
The three promotions clear the three-source bar (ACR-001 at four
sources, ACR-002 and ACR-004 at five, §§8--9), so the cycle exercised
admission above the bar but never rejection just below it. The
protocol's discriminative power against single-condition marginal
failures is therefore established by the threshold's specification, not
by empirical worked decisions; an empirically grounded near-miss
non-admission case is registered as future programme work under the
cycle's reusable workflow {[}4{]}, to surface in subsequent cycle
iterations when a candidate of that profile arises.
